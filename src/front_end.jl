"""
前端 负责关键点跟踪和计算frame pose. 还决定何时需要一个新的关键帧。
# Parameters:
- `current_frame::Frame`: Current frame that is being processed.
    This is a shared Frame between FrontEnd, MapManager, Estimator and SlamManager.
...
mutable struct FrontEnd
    ...
    current_pyramid::LKPyramid  # LK金字塔. 预计算用来优化当前图像光流跟踪的金字塔
...
function track_mono!(fe::FrontEnd, image, image_time, visualizer)
    ...
    set_wc!(fe.current_frame, fe.motion_model(fe.current_frame.wc, image_time), visualizer)
...
Pose is computed from the triangulated Keypoints (MapPoints) that are visible in this frame.
P3P Ransac algorithm. 用5个(3D,2D)点对通过p3p_ransac方法求解外参.
"""
function compute_pose!(fe::FrontEnd, visualizer)
    ...
    for kp in values(fe.current_frame.keypoints)
        kp.is_3d || continue
        mp = get(fe.map_manager.map_points_dict, kp.id, nothing)
        mp ≡ nothing && continue

        p3p_pdn_positions[i] = normalize(kp.position)   # 相机坐标系下的3d points
    ...
    res = p3p_ransac(p3p_3d_points, p3p_pixels, p3p_pdn_positions,
...
function compute_pose_5pt!(fe::FrontEnd; min_parallax, use_motion_model)
    if fe.current_frame.nb_keypoints < 8
        @debug "[FE] Not enough keypoints for initialization:$(fe.current_frame.nb_keypoints)"
        return nothing
    end

    previous_keyframe = get(fe.map_manager.frames_map, fe.current_frame.kfid, nothing)
    ...
    previous_pd = Vector{Point2f}(undef, fe.current_frame.nb_keypoints)  # 归一化的, predivided by K^-1 
    ...
    @inbounds for keypoint in values(fe.current_frame.keypoints)
        ...
        current_pd[i] = keypoint.position[[1, 2]]  # pd: pre divide. 逆投影到相机坐标的3D点. 
        kp_ids[i] = keypoint.id
        i += 1

        # Compute rotation-compensated parallax.
        rot_position = R_compensation * keypoint.position
        rot_px = project(fe.current_frame.camera, rot_position)  # 重投影
        avg_parallax += norm(rot_px - pkf_keypoint.undistorted_pixel)
        n_parallax += 1
    end
    if n_parallax < 8
        @warn "[FE] Not enough keypoints in previous KF to compute 5pt Essential Matrix."
        return nothing
    end

    avg_parallax /= n_parallax
    if avg_parallax < min_parallax
        @warn "[FE] Not enough parallax ($avg_parallax) to compute 5pt Essential Matrix."
        return nothing
    end
    ...
    if n_inliers < 5
        @warn "[FE] Not enough inliers ($n_inliers) for the 5pt Essential Matrix."
        return nothing
    end
    ...
    if use_motion_model
        ...
        prev_cw = get_cw(previous_keyframe)  # 前一帧的 pose
        current = prev_cw * get_wc(fe.current_frame)  # * 当前帧的Pwc
